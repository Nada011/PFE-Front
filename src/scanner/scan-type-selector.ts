...
export class ScanTypeSelector {
  ...
  constructor(supportedScanTypes?: Array<Html5QrcodeScanType> | []) {
    this.supportedScanTypes =
      this.validateAndReturnScanTypes(supportedScanTypes);
  }
  ...
  private validateAndReturnScanTypes(
    ...
    if (supportedScanTypes.length > maxExpectedValues) {
      throw (
        `Max ${maxExpectedValues} values expected for ` + 'supportedScanTypes'
      );
    }
    ...
    for (const scanType of supportedScanTypes) {
      if (
        !Html5QrcodeConstants.DEFAULT_SUPPORTED_SCAN_TYPE.includes(scanType)
      ) {
