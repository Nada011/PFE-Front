...
export class ZXingHtml5QrcodeDecoder implements QrcodeDecoderAsync {
  private readonly formatMap: Map<Html5QrcodeSupportedFormats, any> = new Map([
    ...
    [Html5QrcodeSupportedFormats.CODE_128, ZXing.BarcodeFormat.CODE_128],
    [Html5QrcodeSupportedFormats.DATA_MATRIX, ZXing.BarcodeFormat.DATA_MATRIX],
    [Html5QrcodeSupportedFormats.MAXICODE, ZXing.BarcodeFormat.MAXICODE],
    ...
    for (const requestedFormat of requestedFormats) {
      if (this.formatMap.has(requestedFormat)) {
        ...
        this.logger.logError(
          `${requestedFormat} is not supported by` + 'ZXingHtml5QrcodeShim'
        );
      }
    }
    return zxingFormats;
